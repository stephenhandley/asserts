...
var TestResultsGroup = require('./TestResultsGroup');
...
function dir (options) {
  ...
  FS.readdirSync(options.directory).forEach(function (filename) {
    ...
    var passed = Asserts(test);
    every_test_passed = every_test_passed && passed;
    if (options.after_each) { options.after_each(filepath); }
  });
  
  if (every_test_passed) {
    console.log(Display.stylize(":) ALL TESTS PASSED!", ['bold', 'green']));
  } else {
    console.log(Display.stylize(":( SOME TESTS FAILED.", ['bold', 'red']));
  }
}
...
function _traverse (tests) {
  ...
  for (label in tests) {
    ...
    if (Type.isFunction(test)) {
      ...
      result[label] = _runGroupTest(test); 
...
function _runGroupTest (test) {
  var sub_result = _traverse(test);
  return new TestResultsGroup(_everyTestPassed(sub_result), sub_result);
}

function _runTest (test) {
  var passed, message, result;
  ...
  try {
    ...
    var error_message = '';
    if (error.message) { 
      error_message = Display.stylize(error.message, 'underline') + ": "; 
    }
